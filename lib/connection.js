...
function Connection(options) {
    ...
    instance.connection = net.connect(options, function () {
        // Connected
    ...
    instance.connection.on('end', options.onEnd || defaultOnClose);
 ...
 */
Connection.prototype.subscribeToStream = function(streamName, resolveLinkTos, onEventAppeared, onConfirmed, onDropped, credentials, onNotHandled) {
    ...
    this.sendMessage(correlationId, Commands.SubscribeToStream, credentials, data, function(pkg) {
        switch (pkg.command) {
            ...
            case Commands.NotHandled:
                if (onNotHandled) {
                    var notHandled = Messages.NotHandled.decode(pkg.data);
                    onNotHandled(notHandled);
                }
                break;
            default:
...
/**
 * Initiate persistent subscription for one stream.
 *
 ...
 * @param {CatchUpSubscriptionSettings} settings Settings for this subscription.
 */
Connection.prototype.subscribeToPersistent = function (groupName, streamId, credentials, onEventAppeared, onDropped, settings) {
    if (!settings) settings = new PersistentSubscription.Settings();
    var subscription = new PersistentSubscription.Stream(this, groupName, streamId, credentials, onEventAppeared, onDropped, settings);
    subscription.start();
    return subscription;
};
